...
- [ ] R3. 真实 peer 列表查询接口（例如 `zero_peers`）
...
1. P0：实现真实 peer 详情查询 RPC（`zero_peers`），并补 3 节点互联集成测试。
2. P0：实现心跳与空闲超时清理，防止僵尸连接占满 peer 槽位。
3. P1：实现区块/交易 gossip（含去重缓存与速率限制）。
4. P1：实现最小可用链同步（启动后落后节点追块）。
5. P1：VPS 部署模板（systemd + healthcheck + journald + metrics）。
