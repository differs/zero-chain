...
1. 全功能节点发现与连接治理：当前仅基础发现与连接，缺少成熟的 `discv5/kad` 生态互通、黑名单持久化、DoS 防护与连接策略。
2. 完整同步能力：缺少 `header-first + body + state sync`、快照导入导出、断点续传与重组恢复。
...
- [ ] N4. 完整节点发现协议（Kademlia/discv5 真正上线流量，不是简化 UDP 协议）
...
2. `discovery.start()` 改为实际 UDP 监听任务（简化协议）。
...
1. P0：补齐 `N5-b`（断线重连、黑名单持久化、按 IP 限流）。
2. P0：落地 `N7` 最小可用同步（headers/body + 落后节点追块）。
3. P1：实现 `N6` Gossip（交易/区块去重、背压、速率限制）。
4. P1：推进 `N8-b` 多节点扰动测试（网络分区、重连收敛、一致性断言）。
