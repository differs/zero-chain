...
use zerocore::account::{Account, InMemoryAccountManager, U256};
...
/// 创建测试环境 (带真实状态数据库)
fn setup_realistic_environment(
    tx_count: usize,
) -> (StateDb, TransactionPool, Vec<SignedTransaction>) {
    ...
    // 创建交易池
    let pool = TransactionPool::new(TxPoolConfig::default(), account_manager.clone());
