...
use crate::account::{Account, U256, AccountManager};
...
impl TransactionPool {
    ...
    pub fn add_transaction(&self, tx: SignedTransaction) -> Result<Hash, TransactionError> {
        ...
        self.all_transactions.write().insert(tx_hash, pool_tx.clone());
    ...
    pub fn add_transactions(&self, txs: Vec<SignedTransaction>) -> Result<Vec<Hash>, TransactionError> {
    ...
    fn validate_transaction(&self, tx: &SignedTransaction) -> Result<(), TransactionError> {
        ...
        // Check chain ID
        if tx.tx.chain_id != 10086 {  // Would use actual chain ID
    ...
    fn remove_worst_transactions(&self, count: usize) {
        let mut all_txs: Vec<_> = self.all_transactions
            .read()
            .values()
            .cloned()
            .collect();
    ...
    pub fn clear_old_transactions(&self) {
        ...
        let to_remove: Vec<_> = self.all_transactions
            .read()
            .iter()
